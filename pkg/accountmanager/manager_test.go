...
var _ = Describe("Account Manager", func() {
	Context("Account manager workflow", func() {
		...
		It("Add/Remove Selector config", func() {
			...
			err := accountManager.addSelectorConfig(&testAccountNamespacedName, &testCesNamespacedName, ces)
			Expect(err).ShouldNot(HaveOccurred())
			filterConfig := accountManager.getSelectorConfig(&testAccountNamespacedName, &testCesNamespacedName)
			Expect(filterConfig).ShouldNot(BeNil())

			By("Remove selector config")
			accountManager.removeSelectorConfig(&testAccountNamespacedName, &testCesNamespacedName)
			filterConfig = accountManager.getSelectorConfig(&testAccountNamespacedName, &testCesNamespacedName)
